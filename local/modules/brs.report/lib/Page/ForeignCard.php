<?php
	...
	/**
	 * Обработчик страницы отчёта "Консультации по иностранным картам клиентов РСБ".
	 *
	 * Формирует выходные данные (грид, список) в шаблон компонента универсального отчёта.
	 */
	class ForeignCard extends AbstractPage {

		public static string $gridCode = 'brsReportForeignCardList';
		public static string $filterCode = 'brsReportForeignCardFilter';
		...
		private function getSettingsOfFilterGridList(): array {
			...
			foreach (ForeignCardTable::$codeHeaderFields as $columnCode => $columnName) {
		...
		/**
		 * Метод генерирует список из ORM ForeignCardTable.
		 *
		 * @return array
		 */
		public function getDocument(array $arGetListParameters = [], string $columnSort = 'false'): array {
			...
			if ($columnSort != 'false') {
				...
				foreach ($columnSort as $columnCode) {
					$this->columnNameToCode[$columnCode] = ForeignCardTable::$codeHeaderFields[$columnCode];
				}

			} else {
				$this->columnNameToCode = ForeignCardTable::$codeHeaderFields; // массив соответствий названий колонок и кодов
			}
			...
			$foreignCardCollection = ForeignCardTable::getList($arGetListParameters)->fetchAll();

			foreach ($foreignCardCollection as $foreignCard) {

				$row = array();

				$ormCollectValues = $foreignCard;
		...
		private function exportCsv(array $document): void {
			...
			header('Content-Disposition: attachment; filename=Консультации по иностранным картам клиентов РСБ.csv');
		...
		private function exportXls(array $document): void {
			...
			Header('Content-Disposition: attachment;filename=Консультации по иностранным картам клиентов РСБ.xls');
		...
		private function actionRefreshReport(): void {
			...
			\Brs\Report\Agent\ForeignCardTable::init();
		...
		private function setTemplate(): array {
			...
			$stagesNameDb = ForeignCardTable::getList([
				'select' => [
					'STATUS_SDELKI'
				],
				'group' => [
			...
			$dealCategoryList = DealCategoryTable::getList([
				'filter' => [
					'IS_LOCKED' => false,
					'ID' => [47, 46]// только категории "Консультация по картам МБ" и "Консультация по картам БК"
				],
				'order' => [
					'NAME' => 'ASC'
				]
			...
			$navigationRecordCount = ForeignCardTable::getList($navGetListParams)->getSelectedRowsCount();
