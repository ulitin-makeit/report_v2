<?php
	...
	/**
	 * Обработчик страницы отчёта "Агентский отчёт".
	 * 
	 * Формирует выходные данные (грид, список) в шаблон компонента универсального отчёта.
	 */
	class Agent extends AbstractPage {

		public static string $gridCode = 'brsReportAgentList';
		public static string $filterCode = 'brsReportAgentFilter';
		...
		private function exportCsv(array $document): void {
			...
			header('Content-Disposition: attachment; filename=Агентский отчёт.csv');
		...
		private function exportXls(array $document): void {
			...
			Header('Content-Disposition: attachment;filename=Агентский отчёт.xls');
		...
		private function getSettingsOfFilterGridList(): array {
			...
			foreach(AgentTable::$codeHeaderFields as $columnCode => $columnName){
		...
		private function setTemplate(): array {
			...
			$navigationRecordCount = AgentTable::getList($navGetListParams)->getSelectedRowsCount();
		...
		private function getDocument(array $arGetListParameters = [], string $columnSort = 'false'): array {
			...
			if($columnSort != 'false'){
				...
				foreach($columnSort as $columnCode){
					$this->columnNameToCode[$columnCode] = AgentTable::$codeHeaderFields[$columnCode];
				}

			} else {
				$this->columnNameToCode = AgentTable::$codeHeaderFields; // массив соответствий названий колонок и кодов
			}
			...
			$agentCollection = AgentTable::getList($arGetListParameters)->fetchCollection();

			foreach($agentCollection as $agent){

				$row = array();

				$ormCollectValues = $agent->collectValues();
